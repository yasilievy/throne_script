...
class throne_script:
    def __init__(self):
        ...
        self.current_do_option = 0
        self.selected_skill_set = 1

    def on_release(self,key):
        if '4' in '{0}'.format(key) and self.double_click_bool:
        ...
        if key == keyboard.Key.esc:
            if self.do_option:
                ...
                self.do_option = True
        if key == keyboard.Key.f5 and self.do_option:
            self.current_do_option = 1
            self.do_option = False
        if key == keyboard.Key.f6 and self.do_option:
            self.current_do_option = 2
            self.do_option = False
        if key == keyboard.Key.f7 and self.do_option:
